...
{
public:
    void match_request_reader(const eprosima::fastdds::rtps::GuidPrefix_t &guid_prefix,
                              const bool &status)
    {
        matched_status_[guid_prefix].set(request_reader_position, status);
    }

    void match_reply_writer(const eprosima::fastdds::rtps::GuidPrefix_t &guid_prefix,
                            const bool &status)
    {
...
{
public:
    DDSRequestReplyClient(const std::string &service_name)
        : m_participant_(nullptr), request_input_({10, 10}), RequestType_(nullptr),
          ReplyType_(nullptr), m_subscriber_(nullptr), ReplyReader_(nullptr),
          RequestTopic_(nullptr), Replytopic_(nullptr), m_publisher_(nullptr),
          reply_cf_topic_(nullptr), RequestWriter_(nullptr), reply_callback_(nullptr),
                 ...
                 std::function<bool(const T_ReplyType &reply, const SampleInfo &info)> callback)
    {
        ...
        
        {
            ...
            ret = RequestWriter_->write(&request, wparams);
            requests_status_[wparams.sample_identity()] = false;
        }
        return (RETCODE_OK == ret ? true : false);
    }

    bool send_request_for_wait(const T_RequestType &request, T_ReplyType &Reply,
...

        {
            ...
            if (timeout_ms > 0) {
                if (!cv_.wait_for(lock, std::chrono::milliseconds(timeout_ms), [&]() {
                    ...
                    LOG(warning) << "Timeout out waiting for reply.";
                    return false;
                }
            } else {
                cv_.wait(lock, [&]() {
...
private:
    ...
    void create_reply_entities(const std::string &service_name)
    {
        ...
        ReplyReader_ =
            m_subscriber_->create_datareader(Replytopic_, reader_qos, nullptr, StatusMask::none());
