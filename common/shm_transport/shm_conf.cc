...
const uint32_t ShmConf::BLOCK_NUM_8B = 4096;
const uint64_t ShmConf::MESSAGE_SIZE_8B = 8;

const uint32_t ShmConf::BLOCK_NUM_8K = 1024;
const uint64_t ShmConf::MESSAGE_SIZE_8K = 1024 * 8;
...
{
    ceiling_msg_size_ = GetCeilingMessageSize(real_msg_size); //计算每一个块的最大消息大小
    block_buf_size_ = GetBlockBufSize(ceiling_msg_size_);     //计算每一个块的缓冲区大小
    block_num_ = GetBlockNum(ceiling_msg_size_);              //计算块的数量
...
uint64_t ShmConf::GetCeilingMessageSize(const uint64_t &real_msg_size)
{
    ...
    if (real_msg_size <= MESSAGE_SIZE_8B) {
        ceiling_msg_size = MESSAGE_SIZE_8B;
    } else if (real_msg_size <= MESSAGE_SIZE_8K) {
        ceiling_msg_size = MESSAGE_SIZE_8K;
...
uint32_t ShmConf::GetBlockNum(const uint64_t &ceiling_msg_size)
{
    ...
    switch (ceiling_msg_size) {
        ...
        case MESSAGE_SIZE_8B:
            num = BLOCK_NUM_8B;
            break;
        case MESSAGE_SIZE_8K:
            num = BLOCK_NUM_8K;
            break;
