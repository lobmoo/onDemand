...
namespace parser
{
    ...
    static const std::unordered_map<std::string, size_t> basicTypeSizes = {
        ...
        {"DT_TIME", 4},       // 持续时间(ms)，精度为毫秒，4字节
        {"DT_DATE", 4},       // 日期，精度为天，4字节
        {"DT_TOD", 4},        // 时间，只有日时，精度为毫秒，4字节
        {"DT_DT", 4},         // 日期和时间，精度为秒，4字节
        {"DT_LTIME", 8},      // 长持续时间(ms),精度为纳秒，8字节
        {"DT_LDATE", 8},      // 长日期，精度为天，8字节
        {"DT_LTOD", 8},       // 长日时，精度为纳秒，8字节
        {"DT_LDT", 8}         // 长日期和时间，精度为纳秒，8字节
        //ADD
    };
