...
namespace parser
{
            ...
            const std::string &modelVersion)
        {
            ...
            if (itStructNode == structNodes.end()) {
                LOG(warning) << "Failed to find struct node for model: "
            ...
            if (baseTypeNameOptional && baseTypeVersionOptional) {
                ...
                resolveModelMembers(baseTypeKey, allNodes, structNodes, currentModelMembers,
                                    modelSize, offset, baseTypeVersionOptional.get());
            } else if (baseTypeNameOptional) {
                LOG(warning) << "baseType '" << baseTypeNameOptional.get()
                             << "' has no version specified in model: "
                             << currentModelNameAndVersion;
            }
                    ...
                    if (arrayDimensionsOptional) {
                        ...
                        if (memberType != "nonBasic") {
                            ...
                            if (!nonBasicTypeName.empty() && nonBasicTypeVersionOptional) {
                                ...
                                resolveModelMembers(nonBasicKey, allNodes, structNodes,
                                                    nonBasicMembers, nonBasicSize, nonBasicOffset,
                                                    nonBasicVersion);
                                nonBasicSize =
                                ...
                                generateArrayElements = [&](std::vector<int> indices) {
                                    if (indices.size() == dimensions.size()) {
                                        ...
                                        for (auto &child : arrayNode.children) {
                                            child.offset +=
                                                offset - arrayNode.children.front().offset;
                                        }
                        ...
                        if (!nonBasicTypeName.empty() && nonBasicTypeVersionOptional) {
                            ...
                            resolveModelMembers(nonBasicKey, allNodes, structNodes, subMembers,
                                                subSize, subOffset, nonBasicVersion);
                            TreeNode node;
                            node.name = nodeName;
        ...
        bool ModelParser::findNodeAndGetLeaves(const ModelDefine &model,
