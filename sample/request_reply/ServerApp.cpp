...
namespace fastdds
{
    namespace examples
    {
        namespace request_reply
        {
            ...
            ServerApp::ServerApp(const std::string &service_name)
                : participant_(nullptr), request_type_(nullptr), request_topic_(nullptr),
                  subscriber_(nullptr), request_reader_(nullptr), reply_type_(nullptr),
                  reply_topic_(nullptr), publisher_(nullptr), reply_writer_(nullptr), stop_(false)
            {
                ...
                request_reply_info(
                    "ServerApp", "Server initialized with ID: " << participant_->guid().guidPrefix);
            }
            ...
            void ServerApp::run()
            {
                ...
                cv_.wait(lock, [this]() -> bool { return is_stopped(); });
            }
            ...
            void ServerApp::on_participant_discovery(DomainParticipant * /* participant */,
                                                     ...
                                                     bool &should_be_ignored)
            {
                ...
                if (!should_be_ignored) {
                    CLIParser::EntityKind entity_kind =
                        static_cast<CLIParser::EntityKind>(info.user_data.data_vec()[0]);
                ...
                if (!should_be_ignored) {
                    std::string client_str =
                        CLIParser::parse_entity_kind(CLIParser::EntityKind::CLIENT);

                    if (status == rtps::ParticipantDiscoveryStatus::DISCOVERED_PARTICIPANT) {
                        request_reply_debug("ServerApp", client_str
                                                             << " " << status_str << ": "
                                                             << remote_participant_guid_prefix);
                        ...
                        client_matched_status_.match_reply_reader(remote_participant_guid_prefix,
                                                                  false);
                        client_matched_status_.match_request_writer(remote_participant_guid_prefix,
                                                                    false);
            ...
            void ServerApp::on_publication_matched(DataWriter * /* writer */,
                                                   const PublicationMatchedStatus &info)
            {
                std::lock_guard<std::mutex> lock(mtx_);

                rtps::GuidPrefix_t client_guid_prefix =
                    rtps::iHandle2GUID(info.last_subscription_handle).guidPrefix;

                if (info.current_count_change == 1) {
                    request_reply_debug("ServerApp", "Remote reply reader matched with client "
                                                         << client_guid_prefix);
                    client_matched_status_.match_reply_reader(client_guid_prefix, true);
                } else if (info.current_count_change == -1) {
                    request_reply_debug("ServerApp", "Remote reply reader unmatched from client "
                                                         << client_guid_prefix);
            ...
            void ServerApp::on_subscription_matched(DataReader * /* reader */,
                                                    const SubscriptionMatchedStatus &info)
            {
                std::lock_guard<std::mutex> lock(mtx_);

                rtps::GuidPrefix_t client_guid_prefix =
                    rtps::iHandle2GUID(info.last_publication_handle).guidPrefix;

                if (info.current_count_change == 1) {
                    request_reply_debug("ServerApp", "Remote request writer matched with client "
                                                         << client_guid_prefix);
                    client_matched_status_.match_request_writer(client_guid_prefix, true);
                } else if (info.current_count_change == -1) {
                    request_reply_debug("ServerApp", "Remote request writer unmatched from client "
                                                         << client_guid_prefix);
            ...
            void ServerApp::on_data_available(DataReader *reader)
            {
                SampleInfo info;
                auto request = std::make_shared<CalculatorRequestType>();

                while ((!is_stopped())
                       && (RETCODE_OK == reader->take_next_sample(request.get(), &info))) {
                    if ((info.instance_state == ALIVE_INSTANCE_STATE) && info.valid_data) {
                        rtps::GuidPrefix_t client_guid_prefix =
                            rtps::iHandle2GUID(info.publication_handle).guidPrefix;
                        rtps::SequenceNumber_t request_id = info.sample_identity.sequence_number();

                        request_reply_info("ServerApp", "Request with ID '"
                                                            << request_id
            ...
            void ServerApp::create_participant()
            {
                ...
                participant_qos.user_data().data_vec().push_back(
                    static_cast<uint8_t>(CLIParser::EntityKind::SERVER));

                participant_ = factory->create_participant(participant_qos.domainId(),
                                                           participant_qos, this, participant_mask);
            ...
            template <>
            Topic *
            ServerApp::create_topic<CalculatorRequestTypePubSubType>(const std::string &topic_name,
                                                                     TypeSupport &type)
            {
                return detail::create_topic<CalculatorRequestTypePubSubType>(topic_name,
                                                                             participant_, type);
            }

            template <>
            Topic *
            ServerApp::create_topic<CalculatorReplyTypePubSubType>(const std::string &topic_name,
                                                                   TypeSupport &type)
            {
                return detail::create_topic<CalculatorReplyTypePubSubType>(topic_name, participant_,
                                                                           type);
            }

            void ServerApp::create_request_entities(const std::string &service_name)
            {
                request_topic_ = create_topic<CalculatorRequestTypePubSubType>("rq/" + service_name,
                                                                               request_type_);
            ...
            void ServerApp::create_reply_entities(const std::string &service_name)
            {
                reply_topic_ =
                    create_topic<CalculatorReplyTypePubSubType>("rr/" + service_name, reply_type_);
                ...
                reply_writer_ = publisher_->create_datawriter(reply_topic_, writer_qos, nullptr,
                                                              StatusMask::none());
            ...
            bool ServerApp::is_stopped() { return stop_.load(); }
            ...
            void ServerApp::reply_routine()
            {
                while (!is_stopped()) {
                    ...
                    cv_.wait(lock,
                             [this]() -> bool { return requests_.size() > 0 || is_stopped(); });
                    ...
                    if (!is_stopped()) {
                        ...
                        rtps::GuidPrefix_t client_guid_prefix =
                            rtps::iHandle2GUID(request.info.publication_handle).guidPrefix;
                        request_reply_debug("ServerApp", "Processing request from client "
                                                             << client_guid_prefix);
                        ...
                        if (!client_matched_status_.is_fully_unmatched(client_guid_prefix)) {
                            request_reply_info("ServerApp",
                                               "Ignoring request from already gone client "
                                                   << client_guid_prefix);
                            continue;
                        }
                        ...
                        if (!calculate(*request.request, result)) {
                            request_reply_error(
                                "ServerApp", "Failed to calculate result for request from client "
                                                 << client_guid_prefix);
                            continue;
                        }
                        ...
                        rtps::SequenceNumber_t request_id =
                        ...
                        if (RETCODE_OK != reply_writer_->write(&reply, write_params)) {
                            ...
                            request_reply_error("ServerApp",
                                                "Failed to send reply to request with ID '"
                                                    << request_id << "' to client "
                                                    << client_guid_prefix);
                            requests_.push(request);
                        } else {
                            request_reply_info("ServerApp", "Reply to request with ID '"
                                                                << request_id << "' sent to client "
                                                                << client_guid_prefix);
                        }
                    }
                }
            ...
            bool ServerApp::calculate(const CalculatorRequestType &request, std::int32_t &result)
            {
                bool success = true;
