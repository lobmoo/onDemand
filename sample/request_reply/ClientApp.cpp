...
namespace fastdds
{
    namespace examples
    {
        namespace request_reply
        {
            ...
            ClientApp::ClientApp(const CLIParser::config &config, const std::string &service_name)
                : request_input_({config.x, config.y}), participant_(nullptr),
                  request_type_(nullptr), request_topic_(nullptr), publisher_(nullptr),
                  request_writer_(nullptr), reply_type_(nullptr), reply_topic_(nullptr),
                  reply_cf_topic_(nullptr), reply_topic_filter_expression_(""),
                  subscriber_(nullptr), reply_reader_(nullptr), stop_(false)
            {
                create_participant();
                ...
                request_reply_info(
                    "ClientApp", "Client initialized with ID: " << participant_->guid().guidPrefix);
            }
            ...
            ClientApp::~ClientApp()
            {
                if (nullptr != participant_) {
                    ...
                    DomainParticipantFactory::get_shared_instance()->delete_participant(
                        participant_);
                }
            ...
            void ClientApp::run()
            {
                ...
                if (!is_stopped()) {
                    request_reply_debug("ClientApp", "One server is available. Waiting for some "
                                                     "time to ensure matching on the server side");
            ...
            void ClientApp::on_participant_discovery(DomainParticipant * /* participant */,
                                                     ...
                                                     bool &should_be_ignored)
            {
                ...
                if (!should_be_ignored) {
                    std::string server_str =
                        CLIParser::parse_entity_kind(CLIParser::EntityKind::SERVER);

                    if (status == rtps::ParticipantDiscoveryStatus::DISCOVERED_PARTICIPANT) {
                        request_reply_debug("ClientApp", server_str
                                                             << " " << status_str << ": "
                                                             << remote_participant_guid_prefix);
            ...
            void ClientApp::on_publication_matched(DataWriter * /* writer */,
                                                   const PublicationMatchedStatus &info)
            {
                std::lock_guard<std::mutex> lock(mtx_);

                rtps::GuidPrefix_t server_guid_prefix =
            ...
            void ClientApp::on_subscription_matched(DataReader * /* reader */,
            ...
            void ClientApp::on_data_available(DataReader *reader)
            {
                SampleInfo info;
                    ...
                    if ((info.instance_state == ALIVE_INSTANCE_STATE) && info.valid_data) {
                        ...
                        if (requests_status_.end() != request_status) {
                            if (!request_status->second) {
                                ...
                                request_reply_debug(
                                    "ClientApp",
                                    "Duplicate reply received from server "
                                        << server_guid_prefix << " to request with ID '"
                                        << request_status->first.sequence_number()
                                        << "' with result: '" << reply.result() << "'");
                                continue;
                            ...
                            request_reply_error(
                                "ClientApp",
                                "Reply received from server "
                                    << server_guid_prefix << " with unknown request ID '"
            ...
            void ClientApp::create_participant()
            {
                ...
                participant_qos.user_data().data_vec().push_back(
                    static_cast<uint8_t>(CLIParser::EntityKind::CLIENT));
            ...
            template <>
            Topic *
            ClientApp::create_topic<CalculatorRequestTypePubSubType>(const std::string &topic_name,
                                                                     TypeSupport &type)
            {
            ...
            template <>
            Topic *
            ClientApp::create_topic<CalculatorReplyTypePubSubType>(const std::string &topic_name,
                                                                   TypeSupport &type)
            {
            ...
            void ClientApp::create_request_entities(const std::string &service_name)
            {
                ...
                request_writer_ = publisher_->create_datawriter(request_topic_, writer_qos, nullptr,
                                                                StatusMask::none());
            ...
            void ClientApp::create_reply_entities(const std::string &service_name)
            {
                ...
                reply_topic_filter_expression_ =
                    "client_id = '" + TypeConverter::to_string(participant_->guid().guidPrefix)
                    + "'";

                reply_cf_topic_ = participant_->create_contentfilteredtopic(
                    "rr/" + service_name + "_cft", reply_topic_, reply_topic_filter_expression_,
                ...
                reply_reader_ = subscriber_->create_datareader(reply_cf_topic_, reader_qos, nullptr,
                                                               StatusMask::none());
            ...
            bool ClientApp::send_request(const CalculatorRequestType &request)
            {
                ...
                request_reply_info("ClientApp", "Request sent with ID '"
                                                    << wparams.sample_identity().sequence_number()
            ...
            bool ClientApp::is_stopped() { return stop_.load(); }
