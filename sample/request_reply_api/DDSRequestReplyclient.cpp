...
DDSRequestReplyClient::DDSRequestReplyClient(const std::string &service_name)
    : m_participant_(nullptr), request_input_({10, 10}), RequestType_(nullptr), ReplyType_(nullptr),
      m_subscriber_(nullptr), ReplyReader_(nullptr), RequestTopic_(nullptr), Replytopic_(nullptr),
      m_publisher_(nullptr), reply_cf_topic_(nullptr), RequestWriter_(nullptr),
      reply_topic_filter_expression_(""), stop_(false)
{
    create_participant();
...
bool DDSRequestReplyClient::create_participant()
{
    ...
    m_participant_ = factory->create_participant(participant_qos.domainId(), participant_qos, this,
                                                 participant_mask);
...
{
    RequestTopic_ = create_topic<CalculatorRequestTypePubSubType>("rq/" + service_name,
                                                                  m_participant_, RequestType_);
    ...
    RequestWriter_ =
        m_publisher_->create_datawriter(RequestTopic_, writer_qos, nullptr, StatusMask::none());
...
{
    Replytopic_ = create_topic<CalculatorReplyTypePubSubType>("rr/" + service_name, m_participant_,
                                                              ReplyType_);
    ...
    reply_cf_topic_ = m_participant_->create_contentfilteredtopic(
        "rr/" + service_name + "_cft", Replytopic_, reply_topic_filter_expression_,
    ...
    ReplyReader_ =
        m_subscriber_->create_datareader(reply_cf_topic_, reader_qos, nullptr, StatusMask::none());
...
bool DDSRequestReplyClient::is_stopped()
{
    return stop_.load();
}
    ...
    const ParticipantBuiltinTopicData &info, bool &should_be_ignored)
{
    ...
    if (info.user_data.data_vec().size() != 1) {
        ...
        LOG(info) << "ClientApp Ignoring participant with invalid user data "
    ...
    if (!should_be_ignored) {
        if (status == eprosima::fastdds::rtps::ParticipantDiscoveryStatus::DISCOVERED_PARTICIPANT) {
            LOG(debug) << "ClientApp server_str"
                       << "server"
                       << ": " << remote_participant_guid_prefix;
        } else if (status
...
void DDSRequestReplyClient::on_publication_matched(DataWriter * /* writer */,
                                                   const PublicationMatchedStatus &info)
{
    ...
    if (info.current_count_change == 1) {
        ...
        LOG(error) << "ClientApp info.current_count_change is not a valid value for "
                      "SubscriptionMatchedStatus current "
                      "count change";
    }
    cv_.notify_all();
}

void DDSRequestReplyClient::on_subscription_matched(DataReader * /* reader */,
...
void DDSRequestReplyClient::run()
{
    ...
    if (!is_stopped()) {
        LOG(debug) << "ClientApp One server is available. Waiting for some time to ensure matching "
                      "on the server side";
