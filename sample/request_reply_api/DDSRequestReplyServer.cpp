...
DDSRequestReplyServer::DDSRequestReplyServer(const std::string &service_name)
    : m_participant_(nullptr), RequestType_(nullptr), ReplyType_(nullptr), m_subscriber_(nullptr),
      RequestReader_(nullptr), RequestTopic_(nullptr), Replytopic_(nullptr), m_publisher_(nullptr),
      ReplyWriter_(nullptr), stop_(false)
{
...
void DDSRequestReplyServer::create_request_entities(const std::string &service_name)
{
    ...
    RequestReader_ =
        m_subscriber_->create_datareader(RequestTopic_, reader_qos, nullptr, StatusMask::none());
...
bool DDSRequestReplyServer::is_stopped()
{
    return stop_.load();
}
    ...
    while (!is_stopped()) {
        ...
        if (!is_stopped()) {
            ...
            if (!client_matched_status_.is_fully_unmatched(client_guid_prefix)) {
                LOG(info) << "ServerApp Ignoring request from already gone client "
                          << client_guid_prefix;
                continue;
            }

            if (!client_matched_status_.is_matched(client_guid_prefix)) {
                LOG(debug) << "ServerApp Client " << client_guid_prefix
            ...
            if (!calculate(*request.request, result)) {
                LOG(error) << "ServerApp Failed to calculate result for request from client "
                           << client_guid_prefix;
                continue;
            }
            ...
            if (RETCODE_OK != ReplyWriter_->write(&reply, write_params)) {
                LOG(error) << "ServerApp Failed to send reply to request with ID '" << request_id
                           << "' to client " << client_guid_prefix;
                requests_.push(request);
            } else {
                LOG(info) << "ServerApp Reply to request with ID '" << request_id
                          << "' sent to client " << client_guid_prefix;
            }
        }
    }
}
...
void DDSRequestReplyServer::on_publication_matched(DataWriter *writer,
...
void DDSRequestReplyServer::on_subscription_matched(DataReader *reader,
...
void DDSRequestReplyServer::on_data_available(DataReader *reader)
{
    ...
    while ((!is_stopped()) && (RETCODE_OK == reader->take_next_sample(request.get(), &info))) {
        if ((info.instance_state == ALIVE_INSTANCE_STATE) && info.valid_data) {
            ...
            LOG(info) << "ServerApp Request with ID '" << request_id << "' received from client "
                      << client_guid_prefix;
            {
