...
class DDSRequestReplyClient : public DomainParticipantListener {
 public:
  ...
  bool send_request(
      const T_RequestType& request, std::function<void(const T_ReplyType& reply, const SampleInfo& info)> callback) {
  ...
  bool send_request_for_wait(const T_RequestType& request, T_ReplyType& Reply, uint32_t timeout_ms = 0) {
    ReturnCode_t ret = RETCODE_ERROR;
    {
      std::unique_lock<std::mutex> lock(mtx_);
      cv_.wait(lock, [&]() { return server_matched_status_.is_any_server_matched() || is_stopped(); });
