    ...
    if (is_spdp) {
        ...
        if (data.has_participant_name && !data.participant_name.empty()) {
            ...
            if (is_pub || is_sub) {
                ...
                // is also a sub (to handle multiple subs with same prefix)
                auto existing = prefix_to_pub_sub_.find(participant_guid.prefix);
                if (is_pub || existing == prefix_to_pub_sub_.end() ||
                    !discovered_pub_sub_[existing->second]) {
