...
def _find_payload_offset(payload, inline_start, inline_end, is_be):
...
class PcapWorker:
    ...
    def _extract_sn(self, payload, body_offset, is_be, wid_hex):
        try:
            if body_offset + 16 > len(payload):
                return
            sn_offset = body_offset + 12  # extraFlags(2)+octets(2)+readerId(4)+writerId(4) = 12
    ...
    def _decode_data_payload(self, payload, submsg_offset, submsg_len, is_be, wid_hex):
        try:
            ...
            if inline_start >= inline_end or inline_start >= len(payload):
                return
            payload_start = _find_payload_offset(payload, inline_start, inline_end, is_be)
